...
class Crud extends School 
{
    ...
    {
        $data['feetype']    = $this->input->post('feetype');
        $data['feetype_priority']     = $this->input->post('feetype_priority');
        $data['status']         = $this->input->post('status');        
        return $this->db->insert('feetypes', $data);
    }
    public function updateFeetype($feetype_id)
    {
        $data['feetype']    = $this->input->post('feetype');
        $data['feetype_priority']     = $this->input->post('feetype_priority');
        $data['status']         = $this->input->post('status');
        $this->db->where('feetype_id', $feetype_id);
        $this->db->update('feetypes', $data);
    }
    public function deleteFeetype($feetype_id)
    {
        $this->db->where('feetype_id', $feetype_id);
        $this->db->delete('feetypes');
    }      
    public function add_teaching_subject()
    {
        $data['subject_name']    = $this->input->post('subject_name');
        $data['department']     = $this->input->post('department');
        $data['status']         = $this->input->post('status');        
        return $this->db->insert('teaching_subjects', $data);
    }
    public function updateTeaching_subject($teaching_subject_id)
    {
        $data['subject_name']    = $this->input->post('subject_name');
        $data['status']         = $this->input->post('status');        
        $this->db->where('teaching_subject_id', $teaching_subject_id);
        $this->db->update('teaching_subjects', $data);
    }    
    
    public function deleteTeaching_subject($teaching_subject_id)
    {
        $this->db->where('teaching_subject_id', $teaching_subject_id);
        $this->db->delete('teaching_subjects');
    }      
    ...
    public function updateDesignation($designation_id)
    {
        $data['designation']    = $this->input->post('designation');
        $data['priority']       = $this->input->post('priority');
        $data['status']         = $this->input->post('status');        
    ...
    function parent_available_exams($class_id,$subject_id) 
    {
        $running_year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $match = array('running_year' => $running_year, 'class_id' => $class_id, 'subject_id' => $subject_id, 'status' => 'published');
        $this->db->order_by("online_exam_id", "dsc");
    ...
    function get_birthdays()
    {
        $year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $array_users = 0;
        $query_admins = $this->db->query("SELECT admin_id, first_name FROM admin WHERE substring_index(birthday, '/', 1) = ".date('m')."")->num_rows();
        $query_teachers = $this->db->query("SELECT teacher_id, first_name FROM teacher WHERE substring_index(birthday, '/', 1) = ".date('m')."")->num_rows();
        $query_accountant = $this->db->query("SELECT accountant_id, first_name FROM accountant WHERE substring_index(birthday, '/', 1) = ".date('m')."")->num_rows();
        $query_librarian = $this->db->query("SELECT librarian_id, first_name FROM librarian WHERE substring_index(birthday, '/', 1) = ".date('m')."")->num_rows();
    ...
    function get_birthdays_by_month($month)
    {
        $year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $array_users = array();
        $query_admins = $this->db->query("SELECT admin_id, first_name, birthday FROM admin WHERE substring_index(birthday, '/', 1) = ".$month."")->result_array();
        ...
        $query_teachers = $this->db->query("SELECT teacher_id, first_name, birthday FROM teacher WHERE substring_index(birthday, '/', 1) = ".$month."")->result_array();
        ...
        $query_accountant = $this->db->query("SELECT accountant_id, first_name, birthday FROM accountant WHERE substring_index(birthday, '/', 1) = ".$month."")->result_array();
        ...
        $query_librarian = $this->db->query("SELECT librarian_id, first_name, birthday FROM librarian WHERE substring_index(birthday, '/', 1) = ".$month."")->result_array();
    ...
    function get_student_year($type = '', $id = '')
    {
        $stdyear = $this->db->get_where(''.$type.'',array($type."_id" => $id))->row()->stdyear;
        return $stdyear;
    }    
    ...
    function updateFbToken($accessToken)
    {
        ...
        $data['fb_name']    = $_SESSION['userData']['first_name'];
    ...
    {
        $name = $this->db->get_where(''.$type.'',array($type."_id" => $id))->row()->first_name;
        return $name;
    }
    function get_parent_mobile($type = '', $id = '')
    {
        $name = $this->db->get_where(''.$type.'',array($type."_id" => $id))->row()->phone;
        return $name;
    }
    function get_student_mother_name($type = '', $id = '')
    {
        $name = $this->db->get_where(''.$type.'',array($type."_id" => $id))->row()->last_name;
        return $name;
    }
    function get_student_mother_mobile($type = '', $id = '')
    {
        $name = $this->db->get_where(''.$type.'',array($type."_id" => $id))->row()->business_phone;
        return $name;
    }
    function get_admin_role($type = '', $id = '')
    {
        $role = $this->db->get_where(''.$type.'',array($type."_id" => $id))->row()->owner_status;
        switch ($role) {
            case "0":
            case "1":
            return "Super Admin";
    ...
    function get_admin_designation($type = '', $id = '')
    {
        $designation = $this->db->get_where(''.$type.'',array($type."_id" => $id))->row()->designation;
        return $designation;
    }    
    ...
    function get_timeline($date,$student_id)
    {
        ...
        $db = $this->db->query("SELECT title, time_end, date_end,homework_id,wall_type,class_id FROM homework WHERE class_id = '$class_id' AND date_end = '$query_date' UNION SELECT title,time_end,exam_date,online_exam_id,wall_type,class_id FROM online_exam WHERE class_id = '$class_id' AND exam_date = '$query_date_exams' UNION SELECT title,timestamp,publish_date,post_id,wall_type,class_id FROM forum WHERE class_id = '$class_id' AND publish_date BETWEEN '$query_date_forum 00:00:00' AND '$query_date_forum 23:59:59' UNION SELECT title, date, time, live_id, wall_type, class_id FROM live WHERE class_id = '$class_id' AND date  =  '$query_date'")->result_array();    
        return $db;
    }
    ...
    function send_exam_notify()
    {
        ...
        $students = $this->db->get_where('enroll', array('class_id' => $this->input->post('class_id'), 'year' => $year))->result_array();
        foreach($students as $row)
        {
            $notify['user_id'] = $row['student_id'];
            $notify['user_type'] = 'student';
            $notify['url'] = "student/online_exams/".base64_encode($this->input->post('class_id').'-'.$this->input->post('subject_id'));
            $notify['date'] = $this->getDateFormat();
            $notify['time'] = date('h:i A');
            $notify['status'] = 0;
            $notify['type'] = 'exam';
    ...
    function send_forum_notify()
    {
        ...
        foreach($students as $row)
        {
            $notify['user_id'] = $row['student_id'];
            $notify['user_type'] = 'student';
            $notify['url'] = "student/forum/".base64_encode($this->input->post('class_id').'-'.$this->input->post('subject_id'));
            $notify['date'] = $this->getDateFormat();
            $notify['time'] = date('h:i A');
            $notify['status'] = 0;
            $notify['type'] = 'forum';
    ...
    function attendanceUpdate($class_id, $subject_id, $timestamp)
    {

        $sms_status   = $this->db->get_where('settings' , array('type' => 'sms_status'))->row()->description;
        $notify       = $this->db->get_where('settings' , array('type' => 'absences'))->row()->description;
        $attendance_of_students = $this->db->get_where('attendance' , array('class_id'=>$class_id,'subject_id' => $subject_id,'year'=> $this->runningYear,'timestamp'=>$timestamp))->result_array();
    ...
    {
        $data['visitor_id']=$visitor_id= substr(md5(rand(0, 1000000)), 0, 7);
        $data['visitor_name']   = $this->input->post('visitor_name');
        $data['mobile']         = $this->input->post('mobile');
        $data['email']          = $this->input->post('email');
        $data['check_in']       = $this->input->post('check_in');
        $data['meet_whom']      = $this->input->post('meet_whom');
        $data['purpose']        = $this->input->post('purpose');
        $data['from']           = $this->input->post('from');
        $data['remarks']        = $this->input->post('remarks');
        if(!empty($_FILES['visitorphoto']['tmp_name'])){
            $data['visitorphoto']   = $visitor_id.$md5.str_replace(' ', '', $_FILES['visitorphoto']['name']);
        }        
        $this->db->insert('visitors', $data);
        move_uploaded_file($_FILES['visitorphoto']['tmp_name'], 'public/uploads/visitor_image/' .$visitor_id.$md5.str_replace(' ', '', $_FILES['visitorphoto']['name']));
    }
    function updateVisitor($visitor_id)
    { 
        $data[check_out] = date('d-m-Y h:i:sa');
